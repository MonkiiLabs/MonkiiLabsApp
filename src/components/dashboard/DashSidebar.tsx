  ...
  return (
    <div className="space-y-4">
        ...
        <div className="-mt-6 px-4 pb-4">
          ...
          <p className="mt-1 text-xs text-paper-3">
            {isAuthenticated && summary
              ? t("dash.sidebar.heartbeatsActive", {
                  heartbeats: fmt(summary.totalHeartbeats),
                  active: fmt(summary.activeAgents),
                })
              : t("dash.sidebar.connectToSync")}
          </p>
          <Link
          ...
          >
            {t("dash.sidebar.telemetryProfile")} <ArrowUpRight className="h-3 w-3" />
          </Link>
        </div>
      </Panel>
      ...
      {displayList.length > 0 && (
        <Panel>
          <PanelHeader
            title={starredAgents.length > 0 ? t("dash.sidebar.watchlist") : t("dash.sidebar.monitored")}
            action={
              <Link
                to="/dashboard/agents"
                className="text-micro font-semibold text-paper-2 underline-offset-4 hover:text-paper hover:underline"
              >
                {t("dash.sidebar.fleetLink")}
              </Link>
            }
          />
